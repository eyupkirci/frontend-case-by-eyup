...
export default async function BlogDetail({ params }: { params: { slug: string } }) {
   ...
   * this function, html-react-parser package can be imported
   *
   * @param section: IHTMLContent
   * @param index: Number
   */

  const renderHTMLTextContent = (section: IHTMLContent, index: number) => {
    if (section.html.startsWith('<p>')) {
      ...
      return <div key={index}>Problem with HTML string</div>;
    }
  };
          ...
          <div className="w-full">
            {Blog?.content?.map((item, index) => (
              <div className="py-2" key={index}>
                {renderHTMLTextContent(item as IHTMLContent, index)}
              </div>
            ))}
          </div>
        </div>
