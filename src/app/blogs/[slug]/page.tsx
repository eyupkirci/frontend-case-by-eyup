...
export default async function BlogDetail({ params }: { params: { slug: string } }) {
  ...
  return (
    <article className="flex min-h-screen flex-col items-center justify-between dark:bg-white">
      ...
      <section className="w-full bg-white p-6">
        ...
        <h1 className="mb-6 text-4xl font-bold dark:text-black">{Blog?.title}</h1>
      ...
      <section className="w-full bg-[#efeff8] p-6">
        <h2 className="mt-12 py-5 text-3xl md:text-5xl dark:text-black">Gerelateerde blogs</h2>
        ...
        {Similar_Blogs?.items?.length > 1 ? (
          <div className="mx-auto flex w-full max-w-screen-2xl flex-col gap-12 py-5 md:m-auto md:flex-row">
            {Similar_Blogs?.items.map((item: ICard) => <Card key={item._id} data={item} />)}
          </div>
        ) : (
